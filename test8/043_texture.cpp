...
const int TEXTURE_SIZE = 40; // 投げるテクスチャの縦横サイズ
const char* TEXTUREFILE = "texture_bird.png";
#pragma endregion
...
namespace mine{
	...
	public:
		...
		void drawTexture(Mat draw_img, Mat texture_img){
			if(pnow[0]>=TEXTURE_SIZE/2 && pnow[1]>=TEXTURE_SIZE/2 &&
				pnow[0]< (draw_img.cols -TEXTURE_SIZE/2) && pnow[1]< (draw_img.rows -TEXTURE_SIZE/2) ){
				Mat resized_texture(Size(TEXTURE_SIZE, TEXTURE_SIZE), CV_8UC4);
				resize(texture_img, resized_texture, resized_texture.size());

				Mat roi(draw_img, Rect(pnow[0]-TEXTURE_SIZE/2, pnow[1]-TEXTURE_SIZE/2, TEXTURE_SIZE, TEXTURE_SIZE));
				resized_texture.copyTo(roi);
			}
		}
	};
}
...
int main(int argc, char *argv[]){
	...
	//　投げるテクスチャ
	Mat texture_img_buf = imread(TEXTUREFILE, -1);
	Mat texture_fourth_ch(texture_img_buf.size(), CV_8UC1);
	vector<Mat> merges;
	merges.push_back(texture_img_buf);
	merges.push_back(texture_fourth_ch);
	Mat texture_img;
	merge(merges, texture_img);
	#pragma endregion
	...
	while ( 1 ) {
		...
		if(me.IsTracked){ // 画面内に人がいたら
			...
			//　テクスチャの描画
			if(rball.IsExist){
				rball.drawTexture(buf_img, texture_img);
				rball.gonext();
			}
