			...
			if(plhand.x-20>=0 && plhand.x+20<adjusted_camera_img.cols 
				...
				for(int x =plhand.x-20; x<plhand.x+20;x++){
					for(int y=plhand.y-20; y<plhand.y+20; y++){
						adjusted_camera_img.at<Vec4b>(y,x)[0] = color_lhand[0]*alpha + adjusted_camera_img.at<Vec4b>(y,x)[0]*(1-alpha);
						adjusted_camera_img.at<Vec4b>(y,x)[1] = color_lhand[1]*alpha + adjusted_camera_img.at<Vec4b>(y,x)[1]*(1-alpha);
						adjusted_camera_img.at<Vec4b>(y,x)[2] = color_lhand[2]*alpha + adjusted_camera_img.at<Vec4b>(y,x)[2]*(1-alpha);
					}
				}
			}
		}
